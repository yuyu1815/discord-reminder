...
def format_time(setting_type:str,week:str,day:str, time:str):
    if "month" in setting_type:
        ...
        # 第?曜日
        if week:
            formatted_time = f"第{day} {format_week[week]}曜日 {time.split(':')[0]}時{time.split(':')[1]}分{time.split(':')[2]}秒"            # 日
        else:
...
class Reminder:
    ...
    async def setup(self,ctx):
        try:
            self.db.create_table(guild_id=ctx.guild.id)
